...
class PreprocessTests(unittest.TestCase):
    def test_load_point_cloud_generates_synthetic_when_missing(self) -> None:
        config = make_config({"preprocess": {"point_cloud_path": "tests/points.pcd"}})
        recorder = make_recorder(config)
        raw_cloud = load_point_cloud(config, recorder)
        self.assertEqual(raw_cloud.source_path, Path("tests/points.pcd"))
        self.assertEqual(raw_cloud.points.shape, (23294, 3))

    def test_preprocess_point_cloud_produces_normals(self) -> None:
        config = make_config(
            {
                "preprocess": {
                    "point_cloud_path": "tests/points.pcd",
                    "downsample_num": 20,
