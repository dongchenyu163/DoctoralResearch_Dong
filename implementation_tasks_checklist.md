...
* [x] **P1-3**: 实现 $\Omega_g$ (有效把持区域) 索引掩码计算逻辑
...
* [x] **P3-2**: 实现力臂评分 ($E_{pdis}$) 及最终位置分归一化
...
* [x] **P5-1**: 实现 Grasp Matrix $G$ 构造 & 配置读取  
* [x] **P5-2**: 实现伪逆求解与力生成 (Sampling in Friction Cone)  
* [x] **P5-3**: 实现力修正与平衡校验 (Correction & Residual Check)  
* [x] **P5-4**: 实现动力学评分 ($E_{mag}, E_{dir}, E_{var}$) & 归一化
